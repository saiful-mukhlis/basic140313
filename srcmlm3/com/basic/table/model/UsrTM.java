...
//			menuItemSearch.add(item);
			...
			item.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent arg0) {
					...
					colSearch = Usr.PENDIDIKAN_TERAKHIR;
				}
			});
			menuItemSearch.add(item);
	...
	public void actionSearch(){
		...
		if (!tmp.trim().equalsIgnoreCase("")) {
			if (colSearch.equals(Usr.GRP)) {
				actionSearchOneField(colSearch,Grp.NAME, tmp);
			}
			else if (colSearch.equals(Usr.JENIS_PEKERJAAN)) {
				actionSearchOneField(colSearch,JenisPekerjaan.NAMA, tmp);
			}
			else if (colSearch.equals(Usr.STATUS)) {
