...
import com.mlm.db.HistoryBayar;
...
public class PpDao extends DaoAbstract {
	...
	public boolean vCode(ODatabaseDocumentTx db, TextField code){
		if (!(code.getText().trim().equals("") || code.getText().trim().equalsIgnoreCase("Auto"))) {
			long tmp=getCountByColumn(db, Pp.CODE, code.getText());
			if (tmp>0) {
	...
	public boolean vCode(ODatabaseDocumentTx db, TextField code, ODocument model){
		if (!code.getText().equalsIgnoreCase((String) model.field(Pp.CODE))) {
			long tmp=getCountByColumn(db, Pp.CODE, code.getText());
			if (tmp>0) {
				App.showErrorDataSudahAda(db, LPp.CODE);
				return false;
	...
	public PpDao setPelanggan(ODocument o, ODocument pelanggan){
		o.field(Pp.PELANGGAN, pelanggan, OType.LINK);
		return this;
	}
	...
	public ODocument getPelanggan(ODatabaseDocumentTx db, ODocument o){
		if (o.isLazyLoad()) {
			return o.field(Pp.PELANGGAN);
		}else{
			ORecordId id=o.field(Pp.PELANGGAN);
			if (id!=null) {
				if (db==null) {
					db = App.getDbd();
	...
	public PpDao setPaket(ODocument o, ODocument paket){
		o.field(Pp.PAKET, paket, OType.LINK);
		return this;
	}
	...
	public ODocument getPaket(ODatabaseDocumentTx db, ODocument o){
		if (o.isLazyLoad()) {
			return o.field(Pp.PAKET);
		}else{
			ORecordId id=o.field(Pp.PAKET);
			if (id!=null) {
				if (db==null) {
					db = App.getDbd();
					ODatabaseRecordThreadLocal.INSTANCE.set(db);
	...
	public PpDao setBayar(ODocument o, BigDecimal bayar) {
		o.field(Pp.BAYAR, bayar, OType.DECIMAL);
		return this;
	}
	...
	public BigDecimal getBayar(ODocument o) {
		BigDecimal tmp = o.field(Pp.BAYAR);
		return tmp;
	}
	...
	public PpDao setUpLine(ODocument o, ODocument upLine){
		o.field(Pp.UP_LINE, upLine, OType.LINK);
		return this;
	}
	...
	public ODocument getUpLine(ODatabaseDocumentTx db, ODocument o){
		if (o.isLazyLoad()) {
			return o.field(Pp.UP_LINE);
		}else{
			ORecordId id=o.field(Pp.UP_LINE);
			if (id!=null) {
				if (db==null) {
					db = App.getDbd();
					ODatabaseRecordThreadLocal.INSTANCE.set(db);
	...
	public PpDao setJmlDownline(ODocument o, int jmlDownline) {
		o.field(Pp.JML_DOWNLINE, jmlDownline, OType.INTEGER);
		return this;
	}
	...
	public int getJmlDownline(ODocument o) {
		Integer tmp = o.field(Pp.JML_DOWNLINE);
		if (tmp == null) {
			tmp = 0;
		}
		return tmp;
	...
	public PpDao setTglDaftar(ODocument o, Date tglDaftar) {
		o.field(Pp.TGL_DAFTAR, tglDaftar, OType.DATE);
		return this;
	}
	...
	public PpDao setTglLunas(ODocument o, Date tglLunas) {
		o.field(Pp.TGL_LUNAS, tglLunas, OType.DATE);
		return this;
	}
	...
	public PpDao setTglAktif(ODocument o, Date tglAktif) {
		o.field(Pp.TGL_AKTIF, tglAktif, OType.DATE);
		return this;
	}
	...
	public PpDao setStatus(ODocument o, int status) {
		o.field(Pp.STATUS, status, OType.INTEGER);
		return this;
	}
	...
	public List<ODocument> getHistoryBayars(ODatabaseDocumentTx db,ODocument o){
		List<ODocument> tmp=o.field(Pp.HISTORY_BAYARS);
		if (tmp==null) {
			tmp=new ArrayList<>();
		}
		return tmp;
	...
	public PpDao setDownlines(ODocument o, List<ODocument> downlines){
		o.field(Pp.DOWNLINES, downlines, OType.LINKLIST);
		return this;
	}
	public List<ODocument> getDownlines(ODatabaseDocumentTx db,ODocument o){
		List<ODocument> tmp=o.field(Pp.DOWNLINES);
		if (tmp==null) {
			tmp=new ArrayList<>();
		}
		return tmp;
	...
	public ODocument createHistoryBayar(Date tglBayar, BigDecimal jmlBayar, BigDecimal jmlSisa){
		ODocument tmp=new ODocument(HistoryBayar.TABLE);
		tmp.field(HistoryBayar.TGL_BAYAR, tglBayar, OType.DATE);
		tmp.field(HistoryBayar.JML_BAYAR, jmlBayar, OType.DECIMAL);
		tmp.field(HistoryBayar.JML_SISA, jmlSisa, OType.DECIMAL);
		return tmp;
	}
	...
	public PpDao setHistoryBayar(ODatabaseDocumentTx db,ODocument o, Date tglBayar, BigDecimal jmlBayar, BigDecimal jmlSisa){
		...
		o.field(Pp.HISTORY_BAYARS, tmp , OType.EMBEDDEDLIST);
		return this;
	}
	...
	public boolean beforeDelete(ODatabaseDocumentTx db, ODocument o) {
		//pelanggan
		command(db, "update "+Pelanggan.TABLE+" remove "+Pelanggan.PAKETS+" = "+o.getIdentity());
